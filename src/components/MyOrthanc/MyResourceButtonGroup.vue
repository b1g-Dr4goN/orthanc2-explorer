...
export default {
    ...
    watch: {
        async studySeries(newValue, oldValue) {
            ...
            } else if (this.resourceLevel == 'studies') {
                // build the modalitiesList to enable/disable viewers
                let modalitiesSet = new Set();
    ...
    computed: {
        ...
        hasLabelsButton() {
            return this.studiesSourceType == SourceType.LOCAL_ORTHANC &&
                this.uiOptions.EnableEditLabels && this.resourceLevel == 'studies';
        },
        isLabelsEnabled() {
            if (this.resourceLevel == 'studies') {
                return this.selectedStudiesIds.length > 0
        ...
        hasShareButton() {
            return this.studiesSourceType == SourceType.LOCAL_ORTHANC && 
                (this.uiOptions.EnableShares && "authorization" in this.installedPlugins && ['studies', 'study'].includes(this.resourceLevel));
        },
        isShareEnabled() {
            if (this.resourceLevel == 'studies') {
                return this.selectedStudiesIds.length > 0
        ...
        hasStoneViewerButton() {
            return this.studiesSourceType == SourceType.LOCAL_ORTHANC && 
                this.hasStoneViewer && (this.resourceLevel == 'study' || this.resourceLevel == 'studies');
        },
        isStoneViewerButtonEnabled() {
            return (this.resourceLevel == 'study' || (this.resourceLevel == 'studies' && this.selectedStudiesIds.length > 0));
        },
        hasVolView() {
        ...
        hasOhifViewerButton() { // Basic viewer
            ...
            if (modalities.length == 0 && this.resourceLevel != "studies") // we can not check the modalities list for bulk mode
            {
                return false;
            }

            if (this.uiOptions.EnableOpenInOhifViewer3) {
                return this.hasOhifViewer && (this.resourceLevel == 'study' || (this.resourceLevel == 'studies' && this.ohifDataSource == 'dicom-web'));
            } else {
        ...
        hasOhifViewerButtonVr() {
            ...
            if (!(this.modalitiesList.includes("CT") || this.modalitiesList.includes("PT") || this.modalitiesList.includes("MR"))
                && this.resourceLevel != "studies") // we can not check the modalities list for bulk mode)
            {
                return false;
            }
        ...
        hasOhifViewerButtonSeg() {
            ...
            if (this.uiOptions.EnableOpenInOhifViewer3 && this.hasOhifViewer) {
                if (this.resourceLevel == "studies" && this.ohifDataSource == 'dicom-web') {
        ...
        hasOhifViewerButtonMicroscopy() {
            ...
            if (!this.modalitiesList.includes("SM") && this.resourceLevel != "studies") {// we can not check the modalities list for bulk mode)
                return false;
            }
        ...
        hasMedDreamViewerButton() {
            return this.hasMedDreamViewer && (this.resourceLevel == 'study' || this.resourceLevel == 'studies');
        },
        ...
        deleteResourceBody() {
            const texts = {
                ...
                "studies": "delete_studies_body"
            }
            return texts[this.resourceLevel];
        },
        hasDownloadZipButton() {
            return this.studiesSourceType == SourceType.LOCAL_ORTHANC && this.uiOptions.EnableDownloadZip && this.resourceLevel != 'instance' && this.resourceLevel != 'studies';
        },
        hasBulkDownloadZipButton() {
            ...
            return this.studiesSourceType == SourceType.LOCAL_ORTHANC && this.uiOptions.EnableDownloadZip && this.resourceLevel == 'studies'
        },
        isBulkDownloadZipEnabled() {
            return this.selectedStudiesIds.length > 0;
        },
        hasDownloadDicomDirButton() {
            return this.studiesSourceType == SourceType.LOCAL_ORTHANC && this.uiOptions.EnableDownloadDicomDir && this.resourceLevel != 'instance' && this.resourceLevel != 'studies';
        },
        hasBulkDownloadDicomDirButton() {
            ...
            return this.studiesSourceType == SourceType.LOCAL_ORTHANC && this.uiOptions.EnableDownloadDicomDir && this.resourceLevel == 'studies'
        },
        ...
        isRetrieveButtonEnabled() {
            return (this.resourceLevel != 'studies' || (this.resourceLevel == 'studies' && this.selectedStudiesIds.length > 0));
        },
        hasAnonymizationButton() {
            return this.studiesSourceType == SourceType.LOCAL_ORTHANC && this.resourceLevel != 'studies';
        },
        ...
        hasApiViewButton() {
            return  this.resourceLevel != 'studies' && this.studiesSourceType == SourceType.LOCAL_ORTHANC && this.uiOptions.EnableApiViewMenu;
        },
        resourcesOrthancId() {
            if (this.resourceLevel == 'studies') {
...
<template>
    <div>
        ...
        <div class="btn-group" v-if="this.resourceLevel == 'studies'">
        ...
        <div v-if="hasCustomButtons && this.resourceLevel == 'studies'" class="btn-group">
            <div v-for="customButton in CustomButtons" :key="customButton.Id" class>
                <button
                    class="btn btn-sm m-1 btn-secondary" type="button" :disabled="!isDeleteEnabled"

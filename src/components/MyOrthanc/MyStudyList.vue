<script>
import StudyItem from "./MyStudyItem.vue"
import ResourceButtonGroup from "./MyResourceButtonGroup.vue"
...
export default {
    ...
    computed: {
        ...
        studyMainDicomTags() {
            return this.studies.filter(study => this.selectedStudiesIds.includes(study.ID)).map(study => study.MainDicomTags);
        },
        patientMainDicomTags() {
            return this.studies.filter(study => this.selectedStudiesIds.includes(study.ID)).map(study => study.PatientMainDicomTags);
        },
        studySeries() {
            return this.studies.filter(study => this.selectedStudiesIds.includes(study.ID)).map(study => study.Series);
        }
    },
    watch: {
    ...
    methods: {
        ...
        async updateUrl() {
            ...
            let newUrl = "/my-study-list/filtered-studies?" + (new URLSearchParams(query)).toString();
                        ...
                        <div class="container">
                            <div class="row g-1">
                                <div class="col-6 study-list-bulk-buttons">
                                    <ResourceButtonGroup :resourceLevel="'studies'"
                                        :resourceOrthancId="selectedStudiesIds"
                                        :patientMainDicomTags="this.patientMainDicomTags"
                                        :studyMainDicomTags="this.studyMainDicomTags" :studySeries="this.studySeries">
                                    </ResourceButtonGroup>
                                </div>
                                <div class="col-4">
                                    <div v-if="!isSearching && isLoadingLatestStudies"
                                        class="alert alert-secondary study-list-alert" role="alert">
                                        <span v-if="isLoadingLatestStudies"
                                            class="spinner-border spinner-border-sm alert-icon" role="status"
                                            aria-hidden="true"></span>{{
                                                $t('loading_latest_studies') }}
                                    </div>
                                    <div v-else-if="!isSearching && isDisplayingLatestStudies"
                                        class="alert alert-secondary study-list-alert" role="alert">
